...
@section('title', 'ویرایش عمل جراحی')
@section('links')
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="card">
			<form action="{{ route('surgeries.update', $surgery->id) }}" id="surgery" name="surgery" method="post" >
				@csrf
				@method('PATCH')
				<div class="card-body">
					<div class="row">
						<div class="col-md-6">
							<div class="form-group required">
								...
								<input class="form-control" value="{{ $surgery->patient_name }}" placeholder="نام" name="patient_name">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group required">
								<label class="form-label">کدملی بیمار</label>
								<input class="form-control" value="{{ $surgery->patient_national_code }}" type="number" placeholder="نام" name="patient_national_code">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group required">
								<label class="form-label">شماره سند</label>
								<input class="form-control" value="{{ $surgery->document_number }}" type="number" placeholder="نام" name="document_number">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control custom-select select2" name="insurance" data-placeholder="انتخاب بیمه">
									...
									@foreach($insurances as $item)										
										<option value="{{ $item->id }}" @if($insurance == $item->id) selected @endif>
							...
							<div class="form-group required">
								...
								<select class="form-control operations" multiple="multiple" name="operations[]">
									@foreach($operations as $item)										
										<option @if(in_array($item->id, $operationIds)) selected="selected" @endif>{{ $item->name }}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-12">
							<div class="form-group">
								...
								<textarea class="content" name="description" id="example">{{ $surgery->description }}</textarea>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group required">
								...
								<div class="input-group">
									...
									</div><input class="form-control fc-datepicker hasDatepicker" value="{{ $surgery->surgeried_at }}" id="dp-surgeried-text" placeholder="YYYY/MM/DD" type="text" aria-label="date1" aria-describedby="date1">
									<input type="hidden" value="{{ $surgery->surgeried_at }}" id="dp-surgeried-date" name="surgeried_at" aria-label="date11" aria-describedby="date11">
								</div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group required">
								...
								<div class="input-group">
									...
									</div><input class="form-control fc-datepicker hasDatepicker" value="{{ $surgery->released_at }}" id="dp-released-text" placeholder="YYYY/MM/DD" type="text" aria-label="date1" aria-describedby="date1">
									<input type="hidden" value="{{ $surgery->released_at }}" id="dp-released-date" name="released_at" aria-label="date11" aria-describedby="date11">
								</div>
							</div>
						</div>
					</div>
								...
								<div class="form-group @if($roles->required == '1') required @endif">
									...
									data-placeholder="انتخاب {{ $roles->title }}">
										...
										@foreach($roles->doctors as $doctor)										
												<option value="{{ $doctor->id }}, {{ $roles->id }}" 
												@selected($surgery->doctors()->where('doctor_role_id',$roles->id)->get()->contains($doctor->id))>{{ $doctor->name }}</option>
										@endforeach
									</select>
								</div>
							</div>
