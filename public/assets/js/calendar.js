$('#dp-surgeried').MdPersianDateTimePicker({
    targetTextSelector: '#dp-surgeried-text',
    targetDateSelector: '#dp-surgeried-date',
});

$('#dp-surgeried-to').MdPersianDateTimePicker({
    targetTextSelector: '#dp-surgeried-to-text',
    targetDateSelector: '#dp-surgeried-to-date',
});

$('#dp-released').MdPersianDateTimePicker({
    targetTextSelector: '#dp-released-text',
    targetDateSelector: '#dp-released-date',
});
  
$('#dp-released-to').MdPersianDateTimePicker({
    targetTextSelector: '#dp-released-to-text',
    targetDateSelector: '#dp-released-to-date',
});


$('#dp-start').MdPersianDateTimePicker({
    targetTextSelector: '#dp-start-text',
targetDateSelector: '#dp-start-date',
});

$('#dp-end').MdPersianDateTimePicker({
    targetTextSelector: '#dp-end-text',
    targetDateSelector: '#dp-end-date',
});


$('#dp-due-date').MdPersianDateTimePicker({
    targetTextSelector: '#dp-due-date-text',
    targetDateSelector: '#dp-due-date-date',
});
